...
Item {
        ...
        ListView {
            ...
            delegate: Rectangle {
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    onClicked: {
                        root.currentPageIndex = thumbnailFrame.index
                        root.jumpToPage(thumbnailFrame.index)
    ...
    function buildPageRows() {
        ...
        if (layoutMode === "twoPage") {
            ...
            if (separateCoverPage && currentPageIndex === 0) {
                rows.push({ pages: [{ pageIndex: 0, source: sources[0] }] })
                return rows
            }
            ...
            var spread = [{ pageIndex: leftPage, source: sources[leftPage] }]
            if (leftPage + 1 < sources.length)
                spread.push({ pageIndex: leftPage + 1, source: sources[leftPage + 1] })
        ...
        var start = 0
        if (separateCoverPage && sources.length > 0) {
            rows.push({ pages: [{ pageIndex: 0, source: sources[0] }] })
            start = 1
        }

        for (var page = start; page < sources.length; page += 2) {
            var pair = [{ pageIndex: page, source: sources[page] }]
