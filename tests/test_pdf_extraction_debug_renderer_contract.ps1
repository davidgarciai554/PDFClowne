...
Assert-Matches $source 'pageHeightPt\s*-\s*pdfRect\.bottom\(\)' 'Debug renderer must invert PDF Y-up coordinates using the higher PDF Y edge as image top.'
Assert-Matches $source 'pdfRect\.left\(\)\s*\*\s*xScale' 'Debug renderer must scale PDF X coordinates to image coordinates.'
