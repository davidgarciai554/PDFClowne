...
Assert-Matches $mainQml 'documentRenderController\.markDocumentOpened\(doc\.path,\s*doc\.renderSessionId\s*\|\|\s*0(?:,\s*doc\.password\s*\|\|\s*"")?\)' 'Overwrite save must restore the render session if the save fails.'
